...
@foreach ($productFilters as  $filter)
    @if(isset($category_id))
            ...
            @if($filterAvailable=="Yes")
                    <div class="col">
                        <label for=">{{ $filter['filter_column'] }}" class="form-label"> Select {{ $filter['filter_name'] }}</label>
                        <select name="{{ $filter['filter_column'] }}" id="{{ $filter['filter_column'] }}" class="form-select">
